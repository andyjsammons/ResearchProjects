    ...
    for spec,vthnm,v,vnum in zip(specnm,vthernm,vnm,range(len(vnm))):
        ...
        for iv in range(1,nv):
            ...
            if reg != 3:
                ...
                abund = abundn[np.where((abundn[v] >= ivel[iv-1]) | (abundn['vhe'] >= ivel[iv-1]) & (abundn[v] < ivel[iv]) | (abundn['vhe'] < ivel[iv]) & (abundn[v] > 0.0)
                & (abundn['he2o'] > 0.0) & (abundn[vthnm] > 0.0) & (regime['flag'] == reg + 1) & (abundn['dnum']
                >= sdnum) & (abundn['dnum'] <= ednum) & (abundn['nhe'] > 0.0))]
            ...
            if reg == 3:
                ...
                abund = abundn[np.where((abundn[v] >= ivel[iv-1]) | (abundn['vhe'] >= ivel[iv-1]) & (abundn[v] < ivel[iv]) | (abundn['vhe'] < ivel[iv]) & (abundn[v] > 0.) & 
                (abundn['he2o'] > 0.) & (abundn[vthnm] > 0.) & (abundn['dnum'] >= sdnum) & (abundn['dnum'] <= ednum)\
                & (abundn['nhe'] > 0.0))]
                
                abund_oxy = abundn[np.where((abundn['vo6'] >= ivel[iv-1]) & (abundn['vo6'] < ivel[iv]) & (abundn['nhe'] > 0.0) & 
                (abundn['he2o'] > 0.0 ) & (abundn['dnum'] >= sdnum) & (abundn['dnum'] <= ednum)& (abundn['vtho6'] > 0.0))]
            ...
            if (np.size(abund) > 0) & (np.size(abund_oxy) > 0):
                ...
                if vnum < 8:
                    ...
                    nvx = np.array([np.sum((((abund[spec] * n * abund['nhe'])/(abund['he2o'])) * ((1.0/(abund[vthnm]*np.sqrt(2.0*np.pi))))) * 
...
np.save('/home/andy/Desktop/pythonstuff/of_vdist',f_vdist)
np.save('/home/andy/Desktop/pythonstuff/ofl_v',fl_v)
