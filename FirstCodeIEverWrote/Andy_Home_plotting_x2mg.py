...
def goodBins(lst1,lst2):
    ...
    arr1 = lst1[np.where((lst1 > 0.0) & (lst2 > 0.0))]
    arr2 = lst2[np.where((lst1 > 0.0) & (lst2 > 0.0))]
...
def histWRanges(flagnum,fluence,dataLim,speed=np.arange(200,1825,25)):
    ...
    for i in range(1,len(speed)):
        ind = np.where((regime['v_p'] >= speed[i-1]-12.5) & (regime['v_p'] <= speed[i]-12.5) & (regime['flag'] == flagnum) & (regime['v_p'] != np.inf))
